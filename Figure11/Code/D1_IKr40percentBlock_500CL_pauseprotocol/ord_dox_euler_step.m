function [v_out, cai_out, t_out, V_trace, Cai_trace, Psi_trace, Cam_trace, ROSi_trace, ICaL_trace] = ...
    ord_dox_euler_step(sc, n_beats_SS, n_beats_out, D)
% ORD_DOX_EULER_STEP
% Runs the ORd+DOX Euler loop (exact copy of HZ_DOX_FinalCode.m)
% with 12 conductance scaling factors injected.
%
% Inputs:
%   sc          [1x12] scaling factors (baseline = 1.0)
%               [GNa Gto GNaL PCap GKs GK1 Gncx Pnak GKb PNab PCab GpCa]
%   n_beats_SS  number of beats to run to steady state
%   n_beats_out number of final beats to return traces for
%   D           DOX level (0 or 1)
%
% Outputs:
%   v_out       [Nx1] voltage trace of last n_beats_out beats (mV)
%   cai_out     [Nx1] cai trace of last n_beats_out beats (mM)
%   t_out       [Nx1] time vector reset to 0 at start of saved beats (ms)
%   V_trace     same as v_out (alias for compatibility)
%   Cai_trace   same as cai_out

%% --- Scaling factors ---
sc_GNa  = sc(1);   sc_Gto  = sc(2);   sc_GNaL = sc(3);
sc_PCa = sc(4);   sc_GKs  = sc(5);   sc_GK1  = sc(6);
sc_Gncx = sc(7);   sc_Pnak = sc(8);   sc_GKb  = sc(9);
sc_PNab = sc(10);  sc_PCab = sc(11);  sc_GpCa = sc(12);

IKr_scaling = 0.6;

extra_ms    = 1000;   % extra time to record after post-pause beat (ms)
pre_pause_ms = 500; 

%% --- DOX ---
alpha_max  = 1.3;   beta_min = 0.75;
alpha_DOX  = 1 + (alpha_max - 1) * D;
beta_DOX   = 1 - (1 - beta_min) * D;

%% --- Pacing settings (match original) ---
CL       = 500;
amp      = -80;
start    = 100;
duration = 0.5;
...
ft_SS  = n_beats_SS  * CL;
ft_out = pre_pause_ms + pause_ms + n_beats_out * CL + extra_ms;
ft     = ft_SS + pause_ms + ft_out;
...
R_=8314.0; T_=310.0; F_=96485.0;
L_cell=0.01; rad=0.0011;
vcell=1000*pi*rad^2*L_cell;
Ageo=2*pi*rad^2+2*pi*rad*L_cell;
Acap=2*Ageo;
vmyo=0.68*vcell; vmito=0.26*vcell; vsr=0.06*vcell;
vnsr=0.0552*vcell; vjsr=0.0048*vcell; vss=0.02*vcell;
ATP=7.7107; ADP=0.2893;
ETC_Leak=0.35*alpha_DOX;

%% --- Initial conditions --- SS: 40% IKr Block D=1 500ms
v=-85.5264; nai=9.13283; nass=9.13353; ki=136.807; kss=136.807;
cai=0.000454213; cass=0.000434286; cansr=4.50027; cajsr=3.12238;
m=0.0094177; hf=0.606243; hs=0.606152; j_gate=0.596787;
hsp=0.356763; jp=0.572583; mL=0.000301263; hL=0.239734; hLp=0.1148;
a=0.0011829; iF=0.999312; iS=0.158841; ap=0.000602773;
iFp=0.999312; iSp=0.174643; d=4.20282e-09; ff=1; fs=0.685767;
fcaf=1; fcas=0.912495; jca=0.930498; nca=0.521836; ffp=0.999768; fcafp=0.999983;
xrf=0.0532683; xrs=0.861599; xs1=0.552495; xs2=0.00353164;
xk1=0.997351; Jrelnp=1.73437e-06; Jrelp=2.17824e-06; CaMKt=0.264287;
Psi=189.651; ATPm=0.750313; ADPm=9.24966; Cam=0.000531148;
NADHm=2.67146; ROSm=6.35332e-07; ROSi=0.000627203; H2O2=1.18826e-05;
GSH=0.886765;

%% --- Loop variables ---
APD_flag=0; APD=0; t_vdot_max=0; vrest=0; vo=v;
t=0; vdot_old=0; vdot=0; n_beat=0; counter=1; Ist=0;
ENa=0; EK=0; EKs=0;
...
CaMKa=0; CaMKb=0; V_uni=0; V_mNaCa=0; J_mPTP=0;
GNaL_factor=1.0; pca_factor=1.0; Jrel_ROS=1.0; Jup_ROS=1.0;

%% --- Pre-allocate output ---
n_out_steps = round(ft_out / dt);
V_trace   = zeros(n_out_steps, 1);
Cai_trace = zeros(n_out_steps, 1);
t_out_vec = zeros(n_out_steps, 1);
Psi_trace  = zeros(n_out_steps, 1);
Cam_trace  = zeros(n_out_steps, 1);
ROSi_trace = zeros(n_out_steps, 1);
ICaL_trace = zeros(n_out_steps, 1);
out_idx   = 1;

%% =========================================================
%  MAIN EULER LOOP  (exact copy of HZ_DOX_FinalCode.m)
% =========================================================

while t <= ft

    % ROS-dependent scaling
    ...
    ENa = (R_*T_/F_)*log(nao/nai);
    EK  = (R_*T_/F_)*log(ko/ki);
    EKs = (R_*T_/F_)*log((ko+0.01833*nao)/(ki+0.01833*nai));
    ...
    GNaL=0.0075*GNaL_factor*sc_GNaL;
    if celltype==1, GNaL=GNaL*0.6; end
    ...
    if celltype==1, delta_epi=1.0-(0.95/(1.0+exp((v+70.0)/5.0)));
    else, delta_epi=1.0; end
    ...
    AiF=1.0/(1.0+exp((v-213.6)/151.2)); AiS=1.0-AiF;
    ...
    Afcaf=0.3+0.6/(1.0+exp((v-10.0)/10.0)); Afcas=1.0-Afcaf;
    ...
    Kmn=0.002; k2n=1000.0; km2n=jca*1.0;
    ...
    PCaNap=0.00125*PCap; PCaKp=3.574e-4*PCap;
    ...
    Axrf=1.0/(1.0+exp((v+54.81)/38.21)); Axrs=1.0-Axrf;
    ...
    IKr=IKr_scaling*GKr*sqrt(ko/5.4)*xr*rkr*(v-EK);
    ...
    hca=exp((qca*v*F_)/(R_*T_));
    hna=exp((qna*v*F_)/(R_*T_));
    h1=1+nai/kna3*(1+hna); h2=(nai*hna)/(kna3*h1); h3=1.0/h1;
    h4=1.0+nai/kna1*(1+nai/kna2); h5=nai^2/(h4*kna1*kna2); h6=1.0/h4;
    h7=1.0+nao/kna3*(1.0+1.0/hna); h8=nao/(kna3*hna*h7); h9=1.0/h7;
    h10=kasymm+1.0+nao/kna1*(1.0+nao/kna2); h11=nao^2/(h10*kna1*kna2); h12=1.0/h10;
    k1_=h12*cao*kcaon; k2_=kcaoff;
    k3p_=h9*wca; k3pp_=h8*wnaca; k3_=k3p_+k3pp_;
    k4p_=h3*wca/hca; k4pp_=h2*wnaca; k4_=k4p_+k4pp_;
    k5_=kcaoff; k6_=h6*cai*kcaon; k7_=h5*h2*wna; k8_=h8*h11*wna;
    ...
    dn_=x1_+x2_+x3_+x4_;
    E1_=x1_/dn_; E2_=x2_/dn_; E3_=x3_/dn_; E4_=x4_/dn_;
    KmCaAct=150.0e-6;
    ...
    % INaCa_ss
    h1=1+nass/kna3*(1+hna); h2=(nass*hna)/(kna3*h1); h3=1.0/h1;
    h4=1.0+nass/kna1*(1+nass/kna2); h5=nass^2/(h4*kna1*kna2); h6=1.0/h4;
    k1_=h12*cao*kcaon; k2_=kcaoff;
    k3p_=h9*wca; k3pp_=h8*wnaca; k3_=k3p_+k3pp_;
    k4p_=h3*wca/hca; k4pp_=h2*wnaca; k4_=k4p_+k4pp_;
    k5_=kcaoff; k6_=h6*cass*kcaon; k7_=h5*h2*wna; k8_=h8*h11*wna;
    x1_=k2_*k4_*(k7_+k6_)+k5_*k7_*(k2_+k3_);
    x2_=k1_*k7_*(k4_+k5_)+k4_*k6_*(k1_+k8_);
    x3_=k1_*k3_*(k7_+k6_)+k8_*k6_*(k2_+k3_);
    x4_=k2_*k8_*(k4_+k5_)+k3_*k5_*(k1_+k8_);
    ...
    % INaK
    k1p_=949.5; k1m_=182.4; k2p_=687.2; k2m_=39.4;
    k3p_=1899.0; k3m_=79300.0; k4p_=639.0; k4m_=40.0;
    Knai0=9.073; Knao0=27.78; delta_nak=-0.1550;
    Knai=Knai0*exp((delta_nak*v*F_)/(3.0*R_*T_));
    Knao=Knao0*exp(((1.0-delta_nak)*v*F_)/(3.0*R_*T_));
    Kki=0.5; Kko=0.3582; MgADP=0.05; MgATP=9.8; Kmgatp=1.698e-7;
    H_ion=1.0e-7; eP=4.2; Khp=1.698e-7; Knap=224.0; Kxkur=292.0;
    P_nak=eP/(1.0+H_ion/Khp+nai/Knap+ki/Kxkur);
    a1_=(k1p_*(nai/Knai)^3)/((1.0+nai/Knai)^3+(1.0+ki/Kki)^2-1.0);
    b1_=k1m_*MgADP; a2_=k2p_;
...
% PAUSE STIMULUS BLOCK
    t_pause_start = n_beats_SS * CL;
    t_pause_end   = t_pause_start + pause_ms;

    if t <= t_pause_start
        if (t>(start+n_beat*CL)) && (t<(start+duration+n_beat*CL-dt))
            if Ist==0, vrest=v; end
            Ist=amp;
        elseif t>(start+duration+n_beat*CL-dt)
            Ist=0.0;
            n_beat=n_beat+1;
        end
    elseif t <= t_pause_end
        Ist = 0;
    else
        t_post = t - t_pause_end;
        if t_post > start && t_post < (start + duration)
            Ist = amp;
        else
            Ist = 0;
        end
    ...
    if APD_flag==0 && v>-40 && vdot<vdot_old
        t_vdot_max=t-dt; APD_flag=1;
    end
    if APD_flag==1 && v<0.9*vrest
        APD=t-t_vdot_max; APD_flag=0;
    end
    ...
    Jrelnp=Jrelnp+dt*dJrelnp;
    Jrelp=Jrelp+dt*dJrelp;
    Jrel=((1.0-fJrelp)*Jrelnp+fJrelp*Jrelp)*Jrel_ROS;

    Jupnp=0.004375*cai/(cai+0.00092);
    Jupp=2.75*0.004375*cai/(cai+0.00092-0.00017);
    if celltype==1, Jupnp=Jupnp*1.3; Jupp=Jupp*1.3; end
    fJupp=1.0/(1.0+KmCaMK/CaMKa);
    Jleak=0.0039375*cansr/15.0;
    Jup=((1.0-fJupp)*Jupnp+fJupp*Jupp-Jleak)*Jup_ROS;
    Jtr=(cansr-cajsr)/100.0;

    nai=nai+dt*(-(INa+INaL+3.0*INaCa_i+3.0*INaK+INab)*Acap/(F_*vmyo)+JdiffNa*vss/vmyo);
    nass=nass+dt*(-(ICaNa+3.0*INaCa_ss)*Acap/(F_*vss)-JdiffNa);
    ki=ki+dt*(-(Ito+IKr+IKs+IK1+IKb+Ist-2.0*INaK)*Acap/(F_*vmyo)+JdiffK*vss/vmyo);
    kss=kss+dt*(-(ICaK)*Acap/(F_*vss)-JdiffK);
    ...
    cai=cai+dt*(Bcai*(-(IpCa+ICab-2.0*INaCa_i)*Acap/(2.0*F_*vmyo)-Jup*vnsr/vmyo+Jdiff*vss/vmyo-V_uni*vmito/vmyo+V_mNaCa*vmito/vmyo-J_mPTP*vmito/vmyo));

    Bcass=1.0/(1.0+BSRmax*KmBSR/(KmBSR+cass)^2+BSLmax*KmBSL/(KmBSL+cass)^2);
    cass=cass+dt*(Bcass*(-(ICaL-2.0*INaCa_ss)*Acap/(2.0*F_*vss)+Jrel*vjsr/vss-Jdiff));
    cansr=cansr+dt*(Jup-Jtr*vjsr/vnsr);
    Bcajsr=1.0/(1.0+csqnmax*kmcsqn/(kmcsqn+cajsr)^2);
    cajsr=cajsr+dt*(Bcajsr*(Jtr-Jrel));

    % Mitochondria
    F_mito=96.485; R_mito=8.314; C_p=1.8e-3;
    a1_m=120.0; a2_m=3.43; f_m=0.01; NADm_tot=2.970; p_3=0.075; k_mPTP=8.0e-6;
    V_NaCa_max=1.0e-4; b_mito=0.5; deltaPsi0=91; K_Na_m=9.4; K_Ca_m=3.75e-4; n_naca=3;
    V_uni_max=0.0275; K_trans=0.019; K_act=3.8e-4; L_uni=110; n_a=2.8; delta_uni=3e-4;
    K_AGC=0.14e-3; p_4=0.01; q_1=0.2244; q_2=0.1e-3; V_AGC_r=0.025e-3; k_GLY=4.5e-4;
    q_3=100.0e-3; q_4=177.0; q_5=5.0; q_6=10000.0e-3; q_7=190.0; q_8=8.5;
    q_9=0.0020e-3; q_10=-0.030e-3; V_ANT_r=8.123e-3; V_F1F0_r=3.6e-3;
    KATP=3.5e-3; theta_m=0.35; k_ETC=0.764e-3;
    RT_over_F=26.729246592691901; NADPH=1.0; b_ROS=1.0e4; Kcc=0.01;
    G_L_ros=7.82e-8; G_max_ros=7.82e-6; Kappa=70e-3; Em_ROS=4.0;
    k1_SOD=1.2e3; k3_SOD=24; k5_SOD=0.25e-3; etSOD=1.5e-3*beta_DOX;
    Kki_ros=0.5; k1_CAT=17.0; EtCAT=0.01; fr=0.05; EtGPX=0.01;
    Phi1=0.5e-2; Phi2=0.75; k1_GR=5.0e-3; EtGR=0.01;
    Km_GSSG=0.06; Km_NADPH=0.015; GT=1.0; j_Vt2SO2m=0.1;

    V_uni=V_uni_max*(((cai/K_trans)*(1+cai/K_trans)^3*((2*F_mito)*(Psi-deltaPsi0)/(R_mito*T_)))/...
          ((1+cai/K_trans)^4+(L_uni/(1+cai/K_act)^n_a)*(1-exp(-(2*F_mito)*(Psi-deltaPsi0)/(R_mito*T_)))));
    V_mNaCa=V_NaCa_max*(exp((b_mito*F_mito*(Psi-deltaPsi0)/(R_mito*T_)))*log(cai/Cam))/...
    ...
    J_ANT=V_ANT_r*((ADP/(ADP+ATP*exp(-(theta_m*F_mito)*Psi/(R_mito*T_))))-(ADPm/(ADPm+ATPm*exp(((1-theta_m)*F_mito)*Psi/(R_mito*T_)))))*(1/(1+KATP/ADP));
    J_Hleak=q_9*Psi+q_10;
    VIMAC2=-1.0*(1.0e-3+b_ROS/(1.0+Kcc/ROSi))*(G_L_ros+G_max_ros/(1+exp(Kappa*(Em_ROS-(-Psi)))))*(-Psi);
    Vt2SO2m=-j_Vt2SO2m*((-Psi-RT_over_F*log(ROSm/ROSi))/Psi)*(-1*(1.0e-3+b_ROS/(1+Kcc/ROSi))*(G_L_ros+G_max_ros/(1+exp(Kappa*(Em_ROS-(-Psi)))))*(-Psi));
    ...
    dADPm=J_ANT-J_F1F0; dATPm=J_F1F0-J_ANT;
    ...
    dGSH=VGR-VGPX;
    Cam=Cam+dCam*dt; NADHm=NADHm+dNADHm*dt;
    ATPm=ATPm+dATPm*dt; ADPm=ADPm+dADPm*dt;
    Psi=Psi+dPsi*dt; ROSm=ROSm+dROSm*dt;
    ROSi=ROSi+dROSi*dt; H2O2=H2O2+dH2O2*dt; GSH=GSH+dGSH*dt;

    % Save output traces for last n_beats_out beats
    if t >= (ft_SS - pre_pause_ms) && out_idx <= n_out_steps
        V_trace(out_idx)   = v;
        Cai_trace(out_idx) = cai;
        Psi_trace(out_idx)  = Psi;
        Cam_trace(out_idx)  = Cam;
        ROSi_trace(out_idx) = ROSi;
        ICaL_trace(out_idx) = ICaL;
        t_out_vec(out_idx) = t - (ft_SS - pre_pause_ms);
        out_idx = out_idx + 1;
    end
...
v_out   = v;
cai_out = cai;
t_out   = t_out_vec(1:out_idx-1);
V_trace   = V_trace(1:out_idx-1);
Cai_trace = Cai_trace(1:out_idx-1);
Psi_trace  = Psi_trace(1:out_idx-1);
Cam_trace  = Cam_trace(1:out_idx-1);
ROSi_trace = ROSi_trace(1:out_idx-1);
ICaL_trace = ICaL_trace(1:out_idx-1);
