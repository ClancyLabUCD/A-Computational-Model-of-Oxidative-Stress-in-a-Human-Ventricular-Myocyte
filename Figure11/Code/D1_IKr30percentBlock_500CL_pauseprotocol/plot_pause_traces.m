% plot_pause_traces.m
clear, clc, close all
load('outputs_pause_D1IKr30.mat', 'V_all', 'Cai_all', 'Cam_all', 'ROSi_all', 'ICaL_all', 't_grid');
...
title(ax1, 'Post-pause beat — D=1, IKr=30% Block', 'FontSize', 12, 'FontWeight', 'normal')
...
% =========================================================
load('outputs_pause_D1IKr30.mat', 'V_all', 'ICaL_all', 't_grid');

col_V    = [0.4  0.4  0.4];
col_ICaL = [0.2  0.6  0.8];
...
% Time window
t_mask = t_grid >= t_start & t_grid <= t_end;
t_plot = t_grid(t_mask) - t_start;
...
for ii = 1:n_valid
    idx  = valid(ii);
    v    = V_all(idx, t_mask)';
    ical = ICaL_all(idx, t_mask)';
    plot(ax1, t_plot, v,    'Color', [col_V    0.5], 'LineWidth', 0.8)
    plot(ax2, t_plot, ical, 'Color', [col_ICaL 0.5], 'LineWidth', 0.8)
end

ylabel(ax1, 'V_m (mV)',       'FontSize', 12)
ylabel(ax2, 'I_{CaL} (A/F)',  'FontSize', 12)
xlabel(ax2, 'Time (ms)',      'FontSize', 12)

for ax = [ax1 ax2]
    box(ax, 'off')
    set(ax, 'FontSize', 12, 'LineWidth', 1.0, 'TickDir', 'out')
    set(ax, 'XTick', [0 500 1000 1500])
end

title(ax1, 'Post-pause beat — D=1, IKr=40% Block', ...
