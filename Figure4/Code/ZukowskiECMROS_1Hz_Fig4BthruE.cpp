...
// 1Hz, Figure 4B-E
// Looped over cai_scaling values -> separate output files, each restarting
// from the original ICs.
...
void resetState(); //reset all state variables/timers to the original ICs
...
//------------------------------------------------------------------
// cai_scaling conditions to sweep, each with its own output filename.
//------------------------------------------------------------------
...
const ScalingCondition conditions[numConditions] = {
    {1.0, "output3_HZ_1Hz_100perCai.txt"},
    {1.3, "output3_HZ_1Hz_30perCai.txt"},
    {1.5, "output3_HZ_1Hz_50perCai.txt"}
};
...
int main()
{
    ...
    {
        cai_scaling = conditions[c].scaling;

        // Reset every state variable and timer back to the original ICs
