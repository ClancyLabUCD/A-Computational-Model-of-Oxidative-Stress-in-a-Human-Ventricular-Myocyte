...
// Figure 4F: nested sweep over pacing cycle length (CL) and cai_scaling
//
// 4 pacing conditions (CL = 500, 750, 1000, 2000 ms), each with its own
// steady-state initial conditions, x 6 cai_scaling values (1.0-1.5).
// 24 total runs, each restarting from that pacing rate's original ICs.
//
// Output files: output_<CL>CL_<scaling>perCai.txt
...
double cai_scaling = 1.0;

double CL=1000;//pacing cycle length (set per pacing condition in main)
double ft=100*CL;//final time (100 beats, set per pacing condition in main)
...
//------------------------------------------------------------------
// cai_scaling conditions to sweep, each with a label used in the
// output filename (matches the convention already used for Fig 4B-E:
// 1.0 -> "100perCai", 1.X -> "X0perCai").
//------------------------------------------------------------------
...
const int numScaling = 6;
const ScalingCondition scalingConditions[numScaling] = {
    {1.0, "100perCai"},
    {1.1, "10perCai"},
    {1.2, "20perCai"},
    {1.3, "30perCai"},
...
int main()
{
    for (int pc = 0; pc < numPacing; pc++)
    {
        ...
        for (int sc = 0; sc < numScaling; sc++)
        {
            cai_scaling = scalingConditions[sc].scaling;

            // Reset to this pacing rate's original steady-state ICs,
            // then reset all timers/counters for a clean run
            pacingConditions[pc].setIC();
            resetTimers();

            std::string fname = std::string("output_") + pacingConditions[pc].label
                                 + "_" + scalingConditions[sc].label + ".txt";
            ofstream myfile(fname.c_str());

            cout << "Running " << pacingConditions[pc].label << " / "
                 << scalingConditions[sc].label << " (CL=" << CL
                 << ", cai_scaling=" << cai_scaling << ") -> " << fname << endl;
            ...
            while (t<=ft)
            {
                ...
                {
                    myfile << t-(ft-beatssave*CL)<< "\t" << v << "\t" << cai << "\t" << Psi << "\t" << Cam << "\t" << J_ETC << "\t" << ROSi << endl;
...
Bcai=1.0/(1.0+cmdnmax*kmcmdn/pow(kmcmdn+cai,2.0)+trpnmax*kmtrpn/pow(kmtrpn+cai,2.0));
    ...
    cai+=dt*(cai_scaling*Bcai*(-(IpCa+ICab-2.0*INaCa_i)*Acap/(2.0*F*vmyo)-Jup*vnsr/vmyo+Jdiff*vss/vmyo-V_uni*vmito/vmyo + V_mNaCa*vmito/vmyo-J_mPTP*vmito/vmyo));

double Bcass=1.0/(1.0+BSRmax*KmBSR/pow(KmBSR+cass,2.0)+BSLmax*KmBSL/pow(KmBSL+cass,2.0));
cass+=dt*(cai_scaling*Bcass*(-(ICaL-2.0*INaCa_ss)*Acap/(2.0*F*vss)+Jrel*vjsr/vss-Jdiff));
