...
// Figure 8: ETC leak sweep across 4 pacing conditions
//
// For each of 4 pacing conditions (CL = 500, 750, 1000, 2000 ms), sweeps
// ETC_Leak from 0.40 to 0.60 in steps of 0.01. Each leak value runs from
// a starting IC (500CL, 750CL, and 1000CL share the same starting IC by
// design; 2000CL uses its own distinct IC) until the model reaches
// genuine beat-to-beat steady state (peak cai/Cam/ROSi converge within
// SS_THRESH), fails (NaN), hits the ROSm floor, or hits the MAX_BEATS
// safety cap. Because convergence is checked directly rather than
// assumed after a fixed number of beats, each pacing condition settles
// into its own correct steady state regardless of starting IC. A
// failure halts the sweep for that pacing condition (same behavior as
// the original single-CL sweep code) and writes a failure trace file.
//
// Output files (one summary per pacing condition):
...
//
// Failure trace files (only written if a failure actually occurs):
//   failure_trace_<CL>CL_ETC_<leak>.txt
...
const int skip=100;//number of timesetps to skip in sampling of data in output file
const double safetime=25.0;//time from the beginning of each beat during which dt is fixed to small values
const double beatssave=5;//number of beats to save in the output
...
//mito ROS
double NADHm0, Cam0, Psi0, ATPm0, ADPm0, ROSm0, ROSi0, H2O20, GSH0;
...
int main()
{
    for (int pc = 0; pc < numPacing; pc++)
    {
        ...
        std::string summaryName = std::string("ETCLeak_sweep_summary_") + pacingConditions[pc].label + ".txt";
        ofstream summary(summaryName.c_str());
        summary << "ETC_Leak\tstatus\tfail_time_ms\tSS_APD90\tSS_peak_Cai\tSS_peak_Cam\tSS_peak_ROSi\n";

        const double ETC_start = 0.40;
        const double ETC_step  = 0.01;
        const double ETC_max   = 0.60;

        const double SS_THRESH = 1e-7;   // absolute steady-state threshold
        const double PSI_WARN  = 200.0;  // start exporting failure trace once Psi > 200
        const double ROS_FLOOR = 1e-12;  // for failure check only
        const int MAX_BEATS    = 200000;   // safety cap only

        cout << "\n#####################################\n";
        cout << "# Starting sweep for " << pacingConditions[pc].label << " (CL = " << CL << " ms)\n";
        cout << "#####################################\n";

        for (double leak = ETC_start; leak <= ETC_max + 1e-12; leak += ETC_step)
        {
            ETC_Leak = leak;

            cout << "\n=====================================\n";
            cout << "Running " << pacingConditions[pc].label << ", ETC_Leak = " << ETC_Leak << endl;
            cout << "=====================================\n";

            // -----------------------------
            // Reset ORd + mito/ROS state variables to this pacing
            // condition's steady-state IC
            // -----------------------------
            pacingConditions[pc].setIC();

            // cytosolic ATP/ADP constants
            ATP = 7.7107;
            ADP = 0.2893;
            ...
            bool psi_crossed_200 = false;

            double prev_peak_cai  = -1.0;
            double prev_peak_Cam  = -1.0;
            double prev_peak_ROSi = -1.0;

            double curr_peak_cai  = -1.0;
            double curr_peak_Cam  = -1.0;
            double curr_peak_ROSi = -1.0;

            double SS_APD90 = -1.0;
            double SS_peak_cai = -1.0;
            double SS_peak_Cam = -1.0;
            double SS_peak_ROSi = -1.0;

            int current_beat = -1;

            // -----------------------------
            // Failure trace buffer for this ETC leak
            // only written to file if failure actually happens
            // -----------------------------
            ostringstream failbuffer;
            bool started_recording = false;

            // -----------------------------
            // Main time loop: run until SS or failure
            // -----------------------------
            while (true)
            {
                // safety cap only
                if (t > MAX_BEATS * CL) {
                    status = "max_beats_reached";
                    cout << "Reached safety cap at t = " << t << " ms for " << pacingConditions[pc].label
                         << ", ETC_Leak = " << ETC_Leak << endl;
                    break;
                }
                ...
                // -----------------------------------
                // Detect beat transitions and check SS
                // -----------------------------------
                int beat_index = int(t / CL);

                if (beat_index != current_beat) {
                    if (current_beat >= 0) {
                        // compare current beat peaks to previous beat peaks
                        if (prev_peak_cai > 0 && prev_peak_Cam > 0 && prev_peak_ROSi > 0) {
                            double dcai  = fabs(curr_peak_cai  - prev_peak_cai);
                            double dCam  = fabs(curr_peak_Cam  - prev_peak_Cam);
                            double dROSi = fabs(curr_peak_ROSi - prev_peak_ROSi);

                            if (dcai < SS_THRESH && dCam < SS_THRESH && dROSi < SS_THRESH) {
                                ss_reached = true;
                                status = "steady_state";
                                SS_APD90 = APD;
                                SS_peak_cai = curr_peak_cai;
                                SS_peak_Cam = curr_peak_Cam;
                                SS_peak_ROSi = curr_peak_ROSi;

                                cout << "Steady state reached at t = " << t
                                     << " ms for " << pacingConditions[pc].label
                                     << ", ETC_Leak = " << ETC_Leak << endl;
                                break;
                            }
                        }
                        ...
                        // shift peaks
                        prev_peak_cai  = curr_peak_cai;
                        prev_peak_Cam  = curr_peak_Cam;
                        prev_peak_ROSi = curr_peak_ROSi;
                    }

                    // initialize new beat peaks
                    curr_peak_cai  = cai;
                    curr_peak_Cam  = Cam;
                    curr_peak_ROSi = ROSi;
                ...
                // update beat peaks
                if (cai  > curr_peak_cai)  curr_peak_cai  = cai;
                if (Cam  > curr_peak_Cam)  curr_peak_Cam  = Cam;
                if (ROSi > curr_peak_ROSi) curr_peak_ROSi = ROSi;

                // -----------------------------------
                // If Psi crosses 200, start exporting
                // -----------------------------------
                if (Psi > PSI_WARN) {
                    psi_crossed_200 = true;
                }

                if (psi_crossed_200) {
                    if (!started_recording) {
                        failbuffer << "t\tPsi\tROSi\tROSm\tJmPTP\tcai\tCam\n";
                ...
                // Failure checks
                // -----------------------------------
                if (!isfinite(Psi) || !isfinite(ROSi) || !isfinite(ROSm) ||
                    !isfinite(Cam) || !isfinite(cai) || !isfinite(J_mPTP)) {
                    ...
                    cout << "\n=====================================\n";
                    cout << "Failure detected at t = " << fail_time << " ms\n";
                    cout << pacingConditions[pc].label << ", ETC_Leak = " << ETC_Leak << endl;
                    cout << "Psi = " << Psi << endl;
                    cout << "ROSi = " << ROSi << endl;
                    cout << "ROSm = " << ROSm << endl;
                    cout << "J_mPTP = " << J_mPTP << endl;
                    cout << "cai = " << cai << endl;
                    cout << "Cam = " << Cam << endl;
                    cout << "=====================================\n";
                ...
                // optional: classify early ROS depletion if you want
                if (ROSm <= ROS_FLOOR) {
                    failed = true;
                    status = "ROSm_floor";
                    fail_time = t;

                    cout << "\nROSm hit floor at t = " << fail_time
                         << " ms for " << pacingConditions[pc].label
                         << ", ETC_Leak = " << ETC_Leak << endl;
                    break;
                }
            ...
            if (failed) {
                 ...
                 // note: pacingConditions[pc].label already includes "CL"
                 // (e.g. "500CL"), so build the failure filename from the
                 // numeric CL value directly to avoid a "CLCL" duplicate
                 // and to fix the original code's hardcoded "15Hz" bug.
                 char clNumLabel[32];
                 snprintf(clNumLabel, sizeof(clNumLabel), "%d", (int)CL);
                 char fail_filename[200];
                 snprintf(fail_filename, sizeof(fail_filename),
                          "failure_trace_%sCL_ETC_%0.2f.txt", clNumLabel, ETC_Leak);

                 ofstream failtrace(fail_filename);
                 failtrace << failbuffer.str();
                 failtrace.close();

                 // stop sweep at first failed ETC leak for this pacing condition
                 break;
             }
            else if (ss_reached) {
                summary << ETC_Leak << "\t"
...
// -----------------------------------------------------------------
// setIC_500CL()
// Uses the same starting IC as setIC_750CL() and setIC_1000CL() by
// design. Because this sweep runs each leak value to genuine
// beat-to-beat convergence (not a fixed beat count), the model
// reaches its own correct steady state for each pacing rate
// regardless of starting point, within the MAX_BEATS safety cap.
// -----------------------------------------------------------------
void setIC_500CL()
{
    v = -87.4129;
