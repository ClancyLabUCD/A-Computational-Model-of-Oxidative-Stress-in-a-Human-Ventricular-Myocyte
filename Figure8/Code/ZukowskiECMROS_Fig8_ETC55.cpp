...
// Figure 8: representative trace at ETC_Leak = 0.55 across 4 pacing
// conditions.
//
// For each of 4 pacing conditions (CL = 500, 750, 1000, 2000 ms), runs
// 2 beats from that pacing rate's own distinct steady-state IC (at
// ETC_Leak = 0.55, matching the IC used to seed the sweep code at this
// leak value) and saves both beats.
//
// Output files:
//   output_500CL_55.txt
//   output_750CL_55.txt
...
//
// Each file has 5 columns: t, v, cai, Cam, ROSi.
...
double ETC_Leak = 0.55;

//------------------------------------------------------------------
// Pacing conditions to run. Each has its own distinct steady-state IC
// (at ETC_Leak = 0.55) via its ICSetter function pointer, and a label
...
int main()
{
    for (int pc = 0; pc < numPacing; pc++)
    {
        ...
        // Reset to this pacing rate's ETC_Leak=0.55 steady-state IC,
        // then reset all timers/counters for a clean run
        pacingConditions[pc].setIC();
        resetTimers();

        std::string fname = std::string("output_") + pacingConditions[pc].label + "_55.txt";
        ofstream myfile(fname.c_str());

        cout << "Running " << pacingConditions[pc].label
             << " (CL=" << CL << ", ETC_Leak=" << ETC_Leak << ") -> " << fname << endl;
        ...
        while (t<=ft)
        {
            ...
            {
                myfile << t-(ft-beatssave*CL)<< "\t" <<v << "\t" << cai << "\t" << Cam << "\t" << ROSi << endl;
...
void resetTimers()
{
    ...
    // run so every pacing condition starts from an identical, clean
    // simulation
    t = 0;
    t0 = 0;
    counter = 1;
...
void updateMitochondria() {
    ...
    double k_ETC = 0.764E-3; // mM/ms Rate constant of NADH oxidation by ETC
