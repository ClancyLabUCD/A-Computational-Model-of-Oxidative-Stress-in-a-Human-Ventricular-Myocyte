...
// 1Hz, Figure 5
// Looped over 3 fixed ROSi conditions (high, mid, low) -> separate output
// files, each restarting from the original ICs. ROSi is clamped to a
// constant value for the full run (not integrated) and drives
// GNaL_factor, pca_factor, Jrel_ROS, and Jup_ROS.
...
void resetState(double ROSi_const); //reset all state variables/timers to ICs for a fresh run

const double CL=1000;//pacing cycle length
const double ft=200*CL;//final time
...
//------------------------------------------------------------------
// 3 fixed ROSi conditions to sweep. Output files are named
// output_1Hz_ROSihigh.txt, output_1Hz_ROSimid.txt, output_1Hz_ROSilow.txt
//------------------------------------------------------------------
...
const int numConditions = 3;
const ROSiCondition conditions[numConditions] = {
    {0.1,   "output_1Hz_ROSihigh.txt"},
    {1e-3,  "output_1Hz_ROSimid.txt"},
    {1e-4,  "output_1Hz_ROSilow.txt"}
};
...
int main()
{
    ...
    {
        // Reset every state variable and timer back to the model's ICs,
        // clamping ROSi to this condition's fixed value
        resetState(conditions[c].ROSi_const);

        ofstream myfile(conditions[c].filename);

        cout << "Running condition " << (c + 1) << " of " << numConditions
             << " (ROSi = " << conditions[c].ROSi_const << ") -> "
             << conditions[c].filename << endl;
        ...
        while (t<=ft)
        {
            ...
            {
                myfile << t-(ft-beatssave*CL)<< "\t" << v << "\t" << cai << "\t" << cass << "\t" << cansr << "\t" << cajsr << "\t" << Jrel << "\t" << Jup << "\t" << Cam << "\t" << ROSi << endl;
...
void resetState(double ROSi_const)
{
    // ORd state variables
    v = -87.821;
    nai = 7.22427;
    ...
    NADHm = 1.34642;
    ROSm = 1.82385e-06;
    ROSi = ROSi_const;   // ROSi is clamped to this condition's constant
                          // value; it is never integrated (see
                          // updateMitochondria(), where "ROSi += dROSi*dt"
                          // is intentionally left out)
    H2O2 = 7.20523e-06;
    GSH = 0.944713;

    // Timing / counting / stimulus state - must be reset each run
    // so every condition starts from an identical, clean simulation
    t = 0;
    t0 = 0;
    counter = 1;
    n = 0;
