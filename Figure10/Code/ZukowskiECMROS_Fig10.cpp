// C++ Implementation of the Zukowski ECM-ROS Model + DOX
// Figure 9: nested sweep over pacing cycle length (CL) and DOX effect
// (D)
//
// 3 pacing conditions (CL = 500, 750, 1000 ms) x 8 DOX effect levels
// (D = 0, 0.5, 0.6, 0.7, 0.75, 0.8, 0.9, 1.0), plus 1 additional pacing
// condition (CL = 2000 ms) at D=0 and D=1 only (used for the 0.5 Hz
// APD90 validation panel, which only needs the control and maximal-DOX
// endpoints). 26 total runs (3x8 + 2).
// Each run reads its steady-state IC from a text file in the
// InitialConditions/ folder
//
// Expected IC file naming (in ./InitialConditions/):
//   SSIC_<CL>CL_<Dlabel>.txt
//   e.g. SSIC_1000CL_D0.txt, SSIC_750CL_D05.txt, SSIC_500CL_D75.txt
// Each IC file is a list of "name = value;" lines, one per state
// variable (v, nai, nass, ..., GSH), matching the format already in use.
//
// Output files (in the working directory):
//   output_<CL>CL_<Dlabel>.txt
//   e.g. output_1000CL_D0.txt, output_500CL_D75.txt
// Each output file has 33 columns:
//   t, v, cai, cass, cansr, cajsr, Jrel, Jup, INa, INaL, Ito, ICaL,
//   ICaNa, ICaK, IKr, IKs, IK1, INaCa_i, INaCa_ss, INaCa, INaK, IKb,
//   INab, IpCa, ICab, APD, ROSi, ROSm, H2O2, GSH, Psi, NADHm, Cam
...
void resetTimers();
bool loadICFromFile(const std::string& filename);

double D = 0.0;               // DOX effect (set per run in main)
const double alpha_max = 1.3;
const double beta_min  = 0.75;
double alpha_DOX = 1.0;       // recomputed per D in main
double beta_DOX  = 1.0;       // recomputed per D in main

double CL=1000;//pacing cycle length (set per pacing condition in main)
double ft=2*CL;//final time (2 beats, set per pacing condition in main)
...
double ETC_Leak = 0.35; // recomputed per D in main (0.35*alpha_DOX)

//------------------------------------------------------------------
// Pacing condition (500 ms CL only) and DOX effect conditions to
// sweep
//------------------------------------------------------------------
...
struct DoxCondition {
    double Dval;
    const char* label;   // e.g. "D0", "D75"
};

const int numDox = 8;
const DoxCondition doxConditions[numDox] = {
    {0.0,  "D0"},
    {0.5,  "D05"},
    {0.6,  "D06"},
    {0.7,  "D07"},
...
int main()
{
    for (int pc = 0; pc < numPacing; pc++)
    {
        ...
        for (int dc = 0; dc < numDox; dc++)
        {
            D = doxConditions[dc].Dval;
            alpha_DOX = 1 + (alpha_max - 1)*D;
            beta_DOX  = 1 - (1 - beta_min)*D;
            ETC_Leak  = 0.35*alpha_DOX;

            std::string icFile = std::string("InitialConditions/SSIC_")
                                  + pacingConditions[pc].label + "_"
                                  + doxConditions[dc].label + ".txt";

            if (!loadICFromFile(icFile))
            {
                cout << "ERROR: could not load IC file " << icFile
                     << " -- skipping this run." << endl;
                continue;
            }
            ...
            while (t<=ft)
            {
                ...
                {
                    myfile << t-(ft-beatssave*CL)<< "\t" <<v << "\t" <<  cai << "\t" << cass << "\t" << cansr << "\t" << cajsr << "\t" << Jrel << "\t" << Jup << "\t" << INa << "\t" << INaL << "\t" << Ito << "\t" << ICaL << "\t" << ICaNa << "\t" << ICaK << "\t" << IKr << "\t" << IKs << "\t" << IK1 << "\t" << INaCa_i << "\t" << INaCa_ss << "\t" << INaCa << "\t" << INaK << "\t" << IKb << "\t" << INab << "\t" << IpCa << "\t" << ICab << "\t" << APD << "\t" << ROSi << "\t" << ROSm << "\t" << H2O2 << "\t" << GSH << "\t" << Psi << "\t" << NADHm << "\t" << Cam <<  endl;
...
void resetTimers()
{
    ...
    // run so every (CL, D) combination starts from an identical, clean
    // simulation
    t = 0;
    t0 = 0;
    counter = 1;
...
//------------------------------------------------------------------
// loadICFromFile()
// Reads a "name = value;" IC file (one state variable per line) and
// assigns each value to the matching global state variable via a
// name -> pointer lookup table. Returns false if the file can't be
// opened; prints a warning for any line whose variable name isn't
// recognized (typo protection) but continues loading the rest.
//------------------------------------------------------------------
bool loadICFromFile(const std::string& filename)
{
    std::map<std::string, double*> icMap;
    icMap["v"] = &v;
    icMap["nai"] = &nai;
    icMap["nass"] = &nass;
    icMap["ki"] = &ki;
    ...
    while (std::getline(infile, line))
    {
        ...
        size_t semiPos = valueStr.find(';');
        if (semiPos != std::string::npos)
        {
            valueStr = valueStr.substr(0, semiPos);
        }

        if (name.empty() || valueStr.empty()) continue;

        std::map<std::string, double*>::iterator it = icMap.find(name);
        if (it == icMap.end())
        {
            cout << "WARNING: unrecognized IC variable name '" << name
                 << "' in " << filename << " -- skipped." << endl;
            continue;
        }
        ...
        {
            cout << "WARNING: could not parse value for '" << name
                 << "' in " << filename << " -- skipped." << endl;
        }
    }
...
//Parameters from Table 1 ECME RIRR Model for ROS
    ...
    double etSOD = 1.5E-3*beta_DOX; //mM Intraceulluar concentration of SOD (range from 0.5E-3 to 2.5E-3)
