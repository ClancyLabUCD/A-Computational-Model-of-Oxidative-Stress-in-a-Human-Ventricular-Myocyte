...
// Figure 2
// Looped over 8 initial Cai conditions -> Output_Ca1.txt ... Output_Ca8.txt
...
void resetState(double cai0); //reset all state variables/timers to ICs for a fresh run

const double CL=1000;//pacing cycle length
const double ft=5*CL;//final time
...
const double nai_constant = 10;

//------------------------------------------------------------------
// 8 initial Cai conditions to sweep. Output files are named
// Output_Ca1.txt through Output_Ca8.txt, in the same order.
//------------------------------------------------------------------
const int numIC = 8;
const double cai_IC_list[numIC] = {
    0.0395E-3,
    0.1569E-3,
    0.1688E-3,
    0.3001E-3,
...
int main()
{
    ...
    {
        // Build output filename: Output_Ca1.txt, Output_Ca2.txt, ... Output_Ca8.txt
        std::ostringstream fname;
        fname << "Output_Ca" << (ic + 1) << ".txt";
        ofstream myfile(fname.str().c_str());

        // Reset every state variable and timer back to the model's ICs,
        // using this loop's starting cai value
        resetState(cai_IC_list[ic]);

        cout << "Running IC " << (ic + 1) << " of " << numIC
             << " (cai0 = " << cai_IC_list[ic] << ") -> " << fname.str() << endl;
        ...
        while (t<=ft)
        {
            ...
            {
                myfile << t-(ft-beatssave*CL)<< "\t" << v << "\t" << nai << "\t" << cai << "\t" << Cam << endl;
...
void resetState(double cai0)
{
    ...
    // so every IC starts from an identical, clean simulation
    t = 0;
    t0 = 0;
    counter = 1;
    n = 0;
